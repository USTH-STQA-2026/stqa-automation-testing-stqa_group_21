...
# STQA Library Automation — Nhóm 21 (Hoàn thành / Completed)
...
```
stqa-automation-testing-stqa_group_21/
...
├── LICENSE
├── README.md
├── REPORT.md            # Báo cáo kết quả test (bonus B4)
├── docs/                # SRS, BRD, test accounts, đề bài...
└── tests/
    ├── test_login.py           # TC-01, TC-02, TC-03 + TC-13 (B1) + parametrize (B2) ✅
    ├── test_search.py          # TC-04 ~ TC-07 ✅
    ├── test_borrow_return.py   # TC-08 ~ TC-10 + TC-14, TC-15 (B1) ✅
    └── test_general.py         # TC-11 ~ TC-12 + TC-16 (Manual TC-33 / BUG-08 xfail) ✅
```
...
### Cách nhanh cho giảng viên trên Windows

Sau khi giải nén file nộp, mở thư mục dự án và chạy:

```bat
RUN_TESTS.bat
```

File này dùng `.env` đã nộp kèm, tự chuẩn bị `venv`, cài `requirements.txt`, cài Playwright Chromium nếu cần, rồi chạy `pytest`.
...
### Test case bắt buộc (12 TC)

| TC    | Mô tả                                                | File                    | Trạng thái |
| ----- | ---------------------------------------------------- | ----------------------- | ---------- |
| TC-01 | Đăng nhập thành công (*Login success*)               | `test_login.py`         | ✅ PASSED   |
| TC-02 | Đăng nhập thất bại — sai mật khẩu (*Wrong password*) | `test_login.py`         | ✅ PASSED   |
| TC-03 | Đăng nhập thất bại — để trống (*Empty fields*)       | `test_login.py`         | ✅ PASSED   |
| TC-04 | Tìm sách theo tên (*Search by name*)                 | `test_search.py`        | ✅ PASSED   |
| TC-05 | Tìm sách — không có kết quả (*No result*)            | `test_search.py`        | ✅ PASSED   |
| TC-06 | Lọc theo thể loại (*Filter by category*)             | `test_search.py`        | ✅ PASSED   |
| TC-07 | Tìm theo tác giả (*Search by author*)                | `test_search.py`        | ✅ PASSED   |
| TC-08 | Mượn sách (*Borrow a book*)                          | `test_borrow_return.py` | ✅ PASSED   |
| TC-09 | Xem sách đang mượn (*View borrowed books*)           | `test_borrow_return.py` | ✅ PASSED   |
| TC-10 | Trả sách (*Return a book*)                           | `test_borrow_return.py` | ✅ PASSED   |
| TC-11 | Đăng xuất (*Logout*)                                 | `test_general.py`       | ✅ PASSED   |
| TC-12 | Chuyển ngôn ngữ sang EN (*Switch language*)          | `test_general.py`       | ✅ PASSED   |

### Test case nâng cao / Bonus + Manual alignment

| TC    | Mô tả                                                            | File                    | Bonus | Trạng thái |
| ----- | --------------------------------------------------------------- | ----------------------- | ----- | ---------- |
| TC-13 | Đăng nhập với vai trò Thủ thư (*Login as Librarian*)            | `test_login.py`         | B1    | ✅ PASSED   |
| TC-14 | Thành viên bị tạm ngưng mượn sách → bị từ chối (*Suspended*)    | `test_borrow_return.py` | B1    | ✅ PASSED   |
| TC-15 | Thành viên hết hạn mượn sách → bị từ chối (*Expired*)           | `test_borrow_return.py` | B1    | ✅ PASSED   |
